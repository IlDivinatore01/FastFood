...
function showAnalytics(data) {
    dataElements.totalOrders.innerText = 'Total orders: ' + (data.totalOrders || 0);
    dataElements.totalEarned.innerText = 'Total earnings: ' + (data.totalEarned / 100).toFixed(2) + '€';
    dataElements.avgEarned.innerText = 'Average per order: ' + (data.avgEarned / 100).toFixed(2) + '€';
    ...
    if (data.mostOrdered) {
        ...
        dataElements.bestEarned.innerText = 'Earnings: ' + (data.mostOrdered.totalEarned / 100).toFixed(2) + '€';
    } else {
        dataElements.bestName.innerText = 'Best selling dish: None';
        dataElements.bestAmount.innerText = 'Amount: 0';
        dataElements.bestEarned.innerText = 'Earnings: 0.00€';
    }
}