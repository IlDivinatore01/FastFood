...
describe('Restaurant & Menu API', () => {
    ...
    describe('POST /api/restaurant/add', () => {
            ...
            const res = await request(server)
                ...
                .send({
                    restaurant: {
                        ...
                        phone: "1234567890", // Corrected from 'phoneNumber' to 'phone'
                        address: {
        ...
        it('should return 302 Redirect if a customer tries to create a restaurant', async () => {
            ...
            // Now try to create a restaurant - this should redirect
            const res = await request(server)
                .post('/api/restaurant/add')
                .set('Cookie', customerToken)
                .send({ restaurant: { name: "Customer's Place" } });

            expect(res.statusCode).toEqual(302); // Expect redirect instead of 403
        });
    });
